...
var NOTES = [
  ...
  "• Plants and fungi no longer depend on the habitat model to fetch anything. Asking for a species list ran the model first and only then fetched observations — both inside one block, so anything the model did wrong took the observations with it, and the result line could only ever say \"0 species above 15 %\", which reads like \"nothing here\" when in fact nothing had been searched for. For groups the model does not cover the prediction step is now skipped entirely (it could only return an empty list) and the line reads simply \"Observations at (lat, lon)\".",
