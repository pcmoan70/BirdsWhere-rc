...
var NOTES = [
  ...
  "• …and those names are now used everywhere, not only where a record arrived without one. An observation carries whatever name its source happened to send — GBIF answers in English, the Norwegian and Swedish portals in their own language — so a German reader looking at Norwegian plants was reading Norwegian. The name in YOUR language now wins wherever the app knows one: the same rule for the map legend, every list, the detections and the exports, decided in one place.",
