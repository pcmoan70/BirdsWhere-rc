...
var NOTES = [
  ...
  "• Fixed: plants and fungi never appeared in the Images list. That view is built from the list's rows and waits for the observations before drawing cards — but nothing rebuilt it once they arrived, so it kept the \"loading\" placeholder for ever. Birds hid the bug: their list is rebuilt by the fetch anyway. Plants and fungi have no predicted rows at all, so there was nothing else to redraw them. Both now show their species as cards, with the names and photos everything else uses.",
