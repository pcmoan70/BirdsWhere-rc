...
var NOTES = [
  ...
  "• The photo view works properly for plants and fungi. Their cards showed a scientific name and nothing else — the card took its name from the model species' name link, which a plant does not have — and the list behind it was never sorted at all, because sorting deliberately skipped \"not in the model\" rows. Under a bird list those are a handful of oddities at the bottom; for plants and fungi they are EVERY row. Cards now carry the common name and its group icon, and the list ranks by how much has been seen where the model has no probability to rank by.",
  "• One fetch now serves every species group. Until now a fetch retrieved only the group you had selected, so switching from birds to plants left an empty map until you picked the spot again — and a place behaved differently depending on the group and on whether the model covers it. Every fetch now brings back all six kingdoms and the group you have selected simply decides what is SHOWN, so switching is instant and a location behaves the same whatever you are looking at. The trade-off, deliberately taken: each source shares its page budget across six taxa instead of one, so a very busy spot can reach a source's limit sooner than a birds-only fetch did.",
