...
var NOTES = [
  ...
  "• Species names now fill themselves in. Outside birds the name packs are thin — 95 % of insect and amphibian entries were simply the English name, and plants and fungi have no packs at all since they are not in the model — so whenever a species appears without a name in your language the app asks iNaturalist once, quietly, and keeps EVERY language it answers with (not just yours). The names appear as they arrive and are there instantly next time, on this device and for any language you switch to. Settings → Storage → \"Export species names\" hands you the collection as a JSON file: send it in and it grows the packs that ship with the app, for everyone.",
  "• Species photos are sharper and their credits are now complete. The pictures are fetched at 800 px instead of 500 (a card is ~900 device pixels on a modern phone, so 500 was visibly soft), the photographer's name is shown in FULL — it used to be cut at 80 characters, which is not the credit these licences ask for — and the licence itself is now a link to its terms beside the link to the file page. A photo whose licence cannot be positively identified as free is no longer shown at all: everything in a 40-species audit was free (CC BY-SA, CC BY, public domain, CC0), but the app could previously have displayed a non-free file from Wikipedia without noticing.",
  "• Fixed: opening the species list (Images layout included) could throw you straight back to the map a moment later. The list decides between \"show the map and let the dots land\" and \"open the list page\" on EVERY render, not just the first — so any re-render arriving while you were reading (predictions resolving, a late source answering, a layout rebuild) applied the map-first default and closed the page under you. Easiest to hit on the smaller groups, where a second render follows quickly. A list page you opened is never closed by a render you did not ask for.",
  "• Found the dark rectangle on the map — and it was never the map tiles. It is the Species-Range heat layer: leaving Range (or Richness) while the model is still computing let the finished result paint itself anyway, over whatever you had moved on to. What you see is that palette — near-black where the species is unlikely, magenta where it is more likely — laid over the whole computed area, which is why it is a rectangle with a soft edge and why the map underneath shows through unchanged. The heat layer now belongs strictly to those two modes, and leaving one cancels the computation instead of letting it land later.",
].join("\n");
