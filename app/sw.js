...
var NOTES = [
  ...
  "• You can now choose which species types a fetch asks for — Settings → Fetching, \"Species types to fetch\", six ticks, all on to start with. It matters because every source has ONE page budget per fetch, shared across the types you ask for: in a busy place, asking for birds alone brings back more birds than asking for all six. The type you are currently viewing is always fetched and shown ticked and locked, so selecting a type can never give you an empty map. Changing the ticks starts a fresh fetch rather than reusing a narrower one.",
